...
class ExamProvider {
  ...
  Future<List<Exam>> getExamsFromDate(DateTime date) async {
    final db = await database;
    List exams = await db.query(
      TABLE_EXAM_NAME,
      columns: [
  ...
  Future<void> updateExam(Exam exam) async {
    final db = await database;
    await db.update(TABLE_EXAM_NAME, exam.toMap(), where: '$COLUMN_EXAM_ID = ?', whereArgs: [exam.examId]);
  }
